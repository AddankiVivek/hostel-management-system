...
<body>
	...
	<div role="navigation">
		<div class="navbar navbar-inverse">
			<a href="/welcome" class="navbar-brand">Welcome Page To This
				Process</a>
	...
	<c:choose>
					...
					<div class="form-group">
						...
						<div class="col-md-7">
							<input type="text" class="form-control" name="phno"
								value="${user.phno }" pattern="(?=.*[0-9]).{10,}"
							...
							<c:forEach var="user" items="${users }">
								<tr>
									...
									<td>
										<div class="form-group ">
											<a
												href="/Allocate-user?sid=${user.sid }&stdid=${user.stdid}&sname=${user.sname}&lname=${user.lname}">
					...
					<div class="form-group">
						...
						<div class="col-md-7">
							<input type="number" class="form-control" name="phno"
								pattern="[789][0-9]{9}" value="${user.phno }" />
						</div>
					</div>
					<div class="form-group">
		...
		<c:when test="${mode=='MODE_Allocation' }">
			<div class="container text-center">
				...
				<form class="form-horizontal" method="POST" action="save-Allocation">
					<p>This Allocation is happening for${alloc.stdid}whose name is
						${sname}-${lname }</p>

					<input type="hidden" name="sid" value="${alloc.sid }" /> <input
						type="hidden" name="stdid" value="${alloc.stdid }" />
							...
							<c:forEach var="alloc" items="${allocs }">
								<tr>
									...
									<td>
										<div class="form-group ">
											<a
												href="/Deallocating-user?sid=${alloc.sid }&stdid=${alloc.stdid}&room=${alloc.room}&floor=${alloc.floor}&allocdate=${alloc.allocdate}">
												<input type="submit" class="btn btn-primary"
												value="DeAllocation" />
											</a>
										</div>
		...
		<c:when test="${mode=='REPORTS' }">
			<div align = "left" class="container text-left" id="tasksDiv">
			      <font color=black>
				<h3>Search Options</h3>
				<h4 style="color: red">${msg }</h4>
				<hr>
				
				 <input  type = "radio" name="colorRadio" action="/search-by-room"value = "first">Search<br><br><br>
				 <input type = "radio" name="colorRadio" value = "second">Report for Allocated<br><br><br>
				 <input type = "radio" name="colorRadio" value = "third">Report for not Allocated<br>
				
				                    
									 <div class="first box " id = "test1">
						...
						<table class="table table-striped table-bordered">
							...
							<tbody>
								<c:forEach var="alloc" items="${allocs }">
									...
									<div class="second box" id = "test2">
										<a href="/download"> <input type="submit"
											class="btn btn-primary" value="Report of Allocated" />
										</a>
									</div>

									<div class="third box" id = "test3">
										<a href="/download1 "> <input type="submit"
											class="btn btn-primary" value="Report of Not Allocated" />
										</a>
									</div>
					...
					<div class="form-group">
						...
						<div class="col-md-7">
							...
							<input type="text" class="form-control" name="sid"
								value="${alloc.sid}"/ > <input type="hidden"
								name="status" value="Allocated" />
		...
		<c:when test="${mode=='MODE_DEALLOC' }">
			...
			<form class="form-horizontal" method="POST"
				...
				<div class="form-group">
					...
					<input type="hidden" name="stdid" value="${alloc.stdid }" /> <input
						type="hidden" name="floor" value="${alloc.floor }" /> <input
						type="hidden" name="room" value="${alloc.room }" /> <input
						type="hidden" name="allocdate" value="${allocdate }" />
					<div class="form-group ">
		...
		<c:when test="${mode=='MODE_SUCCESS' }">
			<div class="container" id="homediv">
				<div class="jumbotron text-center">
					<h1>Registration was successful for the user ${user.sid}</h1>
					<h3>Whose name is ${user.sname}-${user.lname}</h3>
				</div>
			</div>
